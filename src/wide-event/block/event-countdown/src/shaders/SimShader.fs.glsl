#pragma glslify: Constants = require(./chunks/Constants.glsl)
#pragma glslify: rand = require(./chunks/Rand.glsl)
...
vec2 coords = vUv;
    ...
    coords.y = coords.y * M_PI;         // phi (long)
vec3 sphereCoords = vec3(
        sin(coords.y) * cos(coords.x) * cos(spinParam),
        cos(coords.y),
        sin(coords.y) * sin(coords.x) * sin(spinParam)
    );
